...
class GKIS_gk_stora_Controller {
	// configuration array
	private $config;
	// module info
	private $module;
	...
	function checkImages() {
		...
		if($this->config['generate_thumbnails'] == 1) {
			// basic images params		
			$img_width = $this->config['config']->gk_stora->gk_stora_image_width;
			$img_height = $this->config['config']->gk_stora->gk_stora_image_height;
			$img_bg = $this->config['config']->gk_stora->gk_stora_image_bg;
			$quality = $this->config['config']->gk_stora->gk_stora_quality;
			// check the slides
	...
	function getArticleData() {
		...
		// get the data
		if(count($idsK2) > 0) {
			$this->articlesK2 = GKIS_gk_stora_Model::getDataK2($idsK2);
		}
		if(count($ids) > 0) {
			$this->articles = GKIS_gk_stora_Model::getData($ids);
		}
	}
	// generate view
	function generateView() {
		...
		$document->addStyleDeclaration('
			#gkIs-'.$this->config['module_id'].' { height: '.($this->config['config']->gk_stora->gk_stora_module_height_desktop).'px; }
			@media (max-width: 1040px) {
				#gkIs-'.$this->config['module_id'].' { height: '.($this->config['config']->gk_stora->gk_stora_module_height_tablet).'px; }
			}
			@media (max-width: 640px) {
				#gkIs-'.$this->config['module_id'].' { height: '.($this->config['config']->gk_stora->gk_stora_module_height_mobile).'px; }
			}
		');
		
		// add script fragment
		$document->addScriptDeclaration('try {$Gavick;}catch(e){$Gavick = {};};$Gavick["gkIs-'.$this->config['module_id'].'"] = { "anim_speed": '.$this->config['config']->gk_stora->gk_stora_animation_speed.', "anim_interval": '.$this->config['config']->gk_stora->gk_stora_animation_interval.', "autoanim": '.$this->config['config']->gk_stora->gk_stora_autoanimation.', "slide_links": '.$this->config['config']->gk_stora->gk_stora_slide_links.' };');
		// generate necessary variables
		$width = $this->config['config']->gk_stora->gk_stora_image_width;
		$height = $this->config['config']->gk_stora->gk_stora_image_height;
		// load view
