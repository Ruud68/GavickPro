jQuery(window).on('load',function () {
    setTimeout(function () {
        jQuery('.gkIsWrapper-gk_stora').each(function(i, el) {
            el = jQuery(el);
            var elID = el.attr("id");
            ...
            if (process === imagesToLoad.length) {
                ...
                wrapper.find('.gkIsPagination li').each(function(i, item) {
                    jQuery(item).click(function() {
                        if (i !== $G['actual_slide']) {
                            $G['blank'] = true;
                            gk_stora_autoanimate($G, wrapper, 'next', i);
                        }
                    });
                });
                // auto-animation
                if ($G['autoanim'] == 1) {
                    $G['animation_timer'] = setTimeout(function() {
                        gk_stora_autoanimate($G, wrapper, 'next', null);
                    }, $G['anim_interval']);
                }
                // pagination
...
//                                 $G['blank'] = true;
//                                 gk_stora_autoanimate($G, wrapper, 'prev', null);
//                             } else {
//                                 $G['blank'] = true;
//                                 gk_stora_autoanimate($G, wrapper, 'next', null);
//                             }
...
var gk_stora_animate = function($G, wrapper, imgPrev, imgNext) {
...
var gk_stora_autoanimate = function($G, wrapper, dir, next) {
    ...
    gk_stora_animate($G, wrapper, imgs[i], imgs[next]);
    $G['actual_slide'] = next;
